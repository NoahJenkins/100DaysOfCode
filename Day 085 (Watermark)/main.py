...
def upload_file():
    if request.method == 'POST':
        ...
        if file.filename == '' or not file.filename.lower().rsplit('.', 1)[-1] in {'png', 'jpg', 'jpeg', 'gif'}:
            return redirect(request.url)
        ...
        if file:
            try:
                ...
                return render_template('error.html', error="An error occurred while processing the image.")
